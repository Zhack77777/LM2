...
function showNotification(options) {
    // If this is a confirmation dialog, always show confirm/cancel buttons and do not use timer/toast
    const isConfirm = options.showCancelButton || options.showConfirmButton;
    return Swal.fire({
        title: options.title,
        text: options.text,
        icon: options.icon || 'success',
        toast: !isConfirm && (options.toast || false),
        position: options.position || 'top-end',
        showConfirmButton: isConfirm ? true : (options.showConfirmButton !== undefined ? options.showConfirmButton : false),
        showCancelButton: isConfirm ? true : (options.showCancelButton || false),
        confirmButtonText: options.confirmButtonText || 'OK',
        cancelButtonText: options.cancelButtonText || 'Cancel',
        timer: isConfirm ? undefined : (options.timer || 3000),
...
function loadApplyLeave() {
    ...
    // Load leave types (only assigned to this employee for the current year)
    const leaveTypeSelect = document.getElementById('leaveType');
    const loadedTypes = new Set();
    const user = auth.currentUser;
    if (!user) return;
    const currentYear = new Date().getFullYear();
    // Get assigned leave types for this user (with remainingDays > 0)
    database.ref('leave_balances').orderByChild('userId').equalTo(user.uid).once('value').then(snapshot => {
        const assignedTypeIds = new Set();
        if (snapshot.exists()) {
            snapshot.forEach(balance => {
                const balanceData = balance.val();
                if (balanceData.year === currentYear && balanceData.leaveTypeId && balanceData.remainingDays > 0) {
                    assignedTypeIds.add(balanceData.leaveTypeId);
                }
            });
        }
        // Now load only those leave types
        database.ref('leave_types').once('value').then(typeSnap => {
            while (leaveTypeSelect.options.length > 1) {
                leaveTypeSelect.remove(1);
            }
            let found = false;
            typeSnap.forEach(type => {
                const typeData = type.val();
                const typeId = type.key;
                if (assignedTypeIds.has(typeId) && !loadedTypes.has(typeId)) {
            ...
            if (!found) {
                const option = document.createElement('option');
                option.value = '';
                option.textContent = 'No available leave types';
